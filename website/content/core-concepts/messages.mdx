...
the macros above, these are **fragments** rather than whole messages, so they usually go inside one:
...
They also work on their own, as a locale-aware `Intl` with the view's locale already applied:

```ts
say.number(1234.5); // → "1,234.5" in en, "1 234,5" in fr
say.number(total, { style: '::currency/EUR' }); // → "€1,234.50"
say.date(post.publishedAt, { style: 'long' }); // → "13 September 2026"
say.time(opensAt, { style: 'short' }); // → "19:30"
```

```tsx
<td><Say.Number _={row.total} style="::currency/EUR" /></td>
<time dateTime={post.publishedAt.toISOString()}><Say.Date _={post.publishedAt} style="medium" /></time>
```

Written on its own, with no text around it, a fragment is still formatted for the locale but is not
extracted: it has nothing a translator could change, so it never reaches the catalogue. The
transform writes the ICU into the call instead of an id.
